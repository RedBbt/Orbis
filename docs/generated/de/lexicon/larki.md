<!-- AUTO-GENERATED — DO NOT EDIT DIRECTLY -->
<!-- Erzeugt von tools/documentation/build.py aus language/. Aenderungen gehoeren in die Daten, nicht in diese Datei. -->


# larki

**Stein** — Stein. Regulaeres Nomen der Klasse M-B (Themavokal -i). (Migrierte Woerterbuchglosse aus §24.2; ausformulierte Definition steht aus.)

| Feld | Wert |
|---|---|
| ID | `ORB-LEX-000025` |
| Wortart | nomen (regulaer) |
| Geschlecht | maskulin |
| Klasse | M-B |
| Status | canonical |
| Häufigkeit | UNRATED |
| Domäne | natur |
| Quelle | §24.2 |

## Englisch

**stone**  
*(Übersetzungsstatus: derived; abgeleitet aus der deutschen kanonischen Bedeutung)*

## Deklination

| Kasus | Singular | Plural |
|---|---|---|
| Nominativ | larki | larkiñ |
| Akkusativ | larkin | larkiñin |
| Dativ | larkiş | larkiñiş |
| Genitiv | larkis | larkiñis |

## Lautung und Schrift

- Phonemfolge: l · a · r · k · i
- Silbifizierung: **mehrdeutig** (lar·ki, lark·i) — Befund L-09
- Phonotaktik: ok
- Orbis Manus: blocked (mehrdeutig, 2 Zerlegungen)

## Bedeutung

- Konzept(e): `ORB-CON-000025`

## Etymologie

Keine Herleitung dokumentiert. Die Referenzgrammatik nennt für dieses Lexem keine Proto-Orbis-Form; sie wird nicht rekonstruiert (Status: `unknown`).

## Beispielsätze

- **Xrañ larkiñ girn esten.**  
  Die Steine sind kalt.  
  *The stones are cold.*  
  `ORB-SENT-000065`

## Prüfstand

- Validatorstatus: befund
- Offene Befunde:
  - **L-09** — Keine Silbifizierungs-Praeferenzregel
  - **W-04** — Lexikoneintraege ohne ausformulierte Definition
- Letzte Prüfung: 2026-08-16
