<!-- AUTO-GENERATED — DO NOT EDIT DIRECTLY -->
<!-- Erzeugt von tools/documentation/build.py aus language/. Aenderungen gehoeren in die Daten, nicht in diese Datei. -->


# sarla

**Frau** — Frau. Regulaeres Nomen der Klasse F-A (Themavokal -a). (Migrierte Woerterbuchglosse aus §24.3; ausformulierte Definition steht aus.)

| Feld | Wert |
|---|---|
| ID | `ORB-LEX-000032` |
| Wortart | nomen (regulaer) |
| Geschlecht | feminin |
| Klasse | F-A |
| Status | canonical |
| Häufigkeit | UNRATED |
| Domäne | mensch |
| Quelle | §24.3 |

## Englisch

**woman**  
*(Übersetzungsstatus: derived; abgeleitet aus der deutschen kanonischen Bedeutung)*

## Deklination

| Kasus | Singular | Plural |
|---|---|---|
| Nominativ | sarla | sarlañ |
| Akkusativ | sarlan | sarlañan |
| Dativ | sarlaş | sarlañaş |
| Genitiv | sarlas | sarlañas |

## Lautung und Schrift

- Phonemfolge: s · a · r · l · a
- Silbifizierung: **mehrdeutig** (sar·la, sarl·a) — Befund L-09
- Phonotaktik: ok
- Orbis Manus: blocked (mehrdeutig, 2 Zerlegungen)

## Bedeutung

- Konzept(e): `ORB-CON-000032`

## Etymologie

Keine Herleitung dokumentiert. Die Referenzgrammatik nennt für dieses Lexem keine Proto-Orbis-Form; sie wird nicht rekonstruiert (Status: `unknown`).

## Beispielsätze

- **Xla sarla talat.**  
  Die Frau spricht.  
  *The woman speaks.*  
  `ORB-SENT-000002`
- **Xla sarla dalvat xraş drauneş xnan aulen.**  
  Die Frau gibt dem Vater das Wasser.  
  *The woman gives the father the water.*  
  `ORB-SENT-000025`
- **Xra şaul xlas sarlas loşn est.**  
  Der Name der Frau ist schön.  
  *The name of the woman is beautiful.*  
  `ORB-SENT-000038`
- **Xla loşnla sarla talat.**  
  Die schöne Frau spricht.  
  *The beautiful woman speaks.*  
  `ORB-SENT-000048`
- **Lo xla loşnvaxla sarla est.**  
  Sie ist die schönste Frau.  
  *She is the most beautiful woman.*  
  `ORB-SENT-000055`

## Prüfstand

- Validatorstatus: befund
- Offene Befunde:
  - **L-09** — Keine Silbifizierungs-Praeferenzregel
  - **W-04** — Lexikoneintraege ohne ausformulierte Definition
- Letzte Prüfung: 2026-08-16
